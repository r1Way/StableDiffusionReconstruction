# 本脚本用于基于10个脑区的PCA特征和邻接矩阵，构建更深的图神经网络进行预测。
# 网络结构：2层GCN + 全局池化 + 多层全连接（含激活和Dropout），提升表达能力。
# 用法示例：
# python gnn_predict2.py --subject subj01 --target init_latent
...
class GNNModel(nn.Module):
    """
    更深的图神经网络模型：
    2层GCN + 全局池化 + 4层全连接（含激活和Dropout）
    """
    def __init__(self, in_dim, hidden1=64, hidden2=128, fc1=1024, fc2=1024, fc3=512, fc4=256, out_dim=6400, dropout=0.3):
        super().__init__()
        # 图卷积层
        ...
        self.fc_out = nn.Linear(fc4, out_dim)
...
def main():
    ...
    # 训练
    model.train()
    n_epochs = 1 if args.test else 20  # 建议训练更久
    ...
    np.save(os.path.join(loss_save_dir, f"{subject}_gnn2_epoch_loss_{target}.npy"), np.array(all_epoch_loss))
    np.save(os.path.join(loss_save_dir, f"{subject}_gnn2_batch_loss_{target}.npy"), np.array(all_batch_loss, dtype=object))
    ...
    # 保存测试集预测结果
    save_test_path = os.path.join(decoded_dir, subject, f"{subject}_gnn2_scores_{target}_test.npy")
    np.save(save_test_path, preds)
    print(f"测试集预测结果已保存到: {save_test_path}")

    # 保存模型
    model_path = os.path.join(decoded_dir, subject, f"{subject}_gnn2_model_{target}.pt")
    ...
    save_train_path = os.path.join(decoded_dir, subject, f"{subject}_gnn2_scores_{target}_train.npy")
