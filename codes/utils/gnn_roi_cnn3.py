...
# 本脚本实现：每个脑区先通过4层1D卷积逐步降维（ROICNN），输出20维（可自定义）特征，然后所有脑区特征拼接后进行图卷积（GCN），最后通过全连接层输出目标向量。
# 支持GPU加速，训练过程每个epoch后自动评估训练集表现并保存到CSV。
# 用法示例：
# python gnn_roi_cnn3.py --subject subj01 --target init_latent
# 可选参数：
#   --roi_out_dim  设置ROICNN输出维度（默认20）
#   --test         测试模式，仅训练1个epoch，batch_size=4

class ROICNN(nn.Module):
    """
    更平滑的多层1D卷积网络，每层逐步降维，防止特征突变
    输入: [batch_size, n_voxels]
    输出: [batch_size, out_dim]
    """

    def __init__(self, in_dim, out_dim=20):
        super().__init__()
        # 4层1D卷积，逐步降通道数，最后全局平均池化
        ...
        self.final_fc = nn.Linear(128, out_dim)

    def forward(self, x):
        # x: [batch_size, n_voxels]
        x = x.unsqueeze(1)  # [batch, 1, n_voxels]
        ...
        x = self.final_fc(x)  # [batch, out_dim]
        return x
...
def main():
    ...
    train_csv_path = os.path.join(decoded_dir, subject,
                                  f"{subject}_roi_cnn_gnn3_train_metrics_{target}_outdim{roi_out_dim}.csv")
    ...
    save_test_path = os.path.join(decoded_dir, subject,
                                  f"{subject}_roi_cnn_gnn3_scores_{target}_outdim{roi_out_dim}_test.npy")
    np.save(save_test_path, preds)
    print(f"测试集预测结果已保存到: {save_test_path}")

    model_path = os.path.join(decoded_dir, subject, f"{subject}_roi_cnn_gnn3_model_{target}_outdim{roi_out_dim}.pt")
