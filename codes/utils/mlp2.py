# python mlp2.py --target c --roi ventral  --subject subj01 --batch_size 500 --use_memmap --seed 42 --resume
...
def check_batch_completed(savedir, subject, roi, target, batch_idx):
    model_path = f'{savedir}/{subject}_{"_".join(roi)}_pipeline_{target}_mlp_batch_{batch_idx}.joblib'
...
def train_mlp_batch(X, Y_batch, batch_idx, mlp_params):
    try:
...
def main():
    ...
    mlp_params = {
        "hidden_layer_sizes": (opt.mlp_hidden,),
        "max_iter": opt.mlp_max_iter,
        "alpha": opt.mlp_alpha,
        "activation": opt.mlp_activation,
        "solver": opt.mlp_solver,
    ...
    for batch_idx in range(n_batches):
        ...
        try:
            ...
            model, success = train_mlp_batch(X, Y_batch, batch_idx, mlp_params)
            if success:
                models.append(model)
                batch_info.append((start_idx, end_idx))
                model_path = f'{savedir}/{subject}_{"_".join(roi)}_pipeline_{target}_mlp_batch_{batch_idx}.joblib'
    ...
    if all_predictions:
        ...
        joblib.dump(model_info, f'{savedir}/{subject}_{"_".join(roi)}_model_info_{target}_mlp.joblib')
